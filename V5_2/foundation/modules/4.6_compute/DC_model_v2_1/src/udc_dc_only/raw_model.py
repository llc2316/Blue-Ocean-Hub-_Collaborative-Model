...
def solve_dc_only(bundle: InputBundle, config: dict[str, Any]) -> ModelResult:
    ...
    it_power[~online] = 0.0
    cooling_power[~online] = 0.0
    dc_power[~online] = 0.0
    pue[~online] = np.nan
    service_rate[~online] = 0.0
    ...
    max_base_shortfall = float(
        np.max(
            np.maximum(
                base_dc_power * online.astype(float) - available_power, 0.0
            )
        )
    )
