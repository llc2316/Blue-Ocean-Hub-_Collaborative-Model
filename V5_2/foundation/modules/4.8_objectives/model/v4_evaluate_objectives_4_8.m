...
peakH2InventoryKg=max([cfg.hydrogen.storageInitialKg; ...
    double(p45.state.h2InventoryKg(:))]);
h2StorageDesign=v4_hydrogen_storage_sizing_4_8(cfg,peakH2InventoryKg);
assetContext=struct( ...
    'hydrogenStorageInstalledKg',h2StorageDesign.installedKg, ...
    'hydrogenStorageSizingRule',h2StorageDesign.sizingRule);
[lifecycle,lifecycleAssetTable]=v4_lifecycle_economics_4_8( ...
    cfg,params,operatingCostCNY,revenue,penaltyCostCNY,N*dt,assetContext);
...
packet.service.peakH2InventoryKg=peakH2InventoryKg;
packet.service.hydrogenStorageDesign=h2StorageDesign;
...
packet.audit.hydrogenStorageSizingBoundary=[ ...
    'Installed economic storage capacity equals max(rated-production process ', ...
    'buffer, maximum committed inventory after actual residual-energy dispatch). ', ...
    'The process-buffer duration is [假设值，待企业调研校准].'];
