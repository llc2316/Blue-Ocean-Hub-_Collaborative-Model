...
> **2026-07-31结构迁移：** 48 h场景、联合模型验证、22方案、第5章准备代码/文档及
> 全部V5验证实现已拆分至兄弟项目`../../C5/`。本目录只保留V5物理模型、配置、冻结接口、
> 供能适配、基础证据和求解内核；活跃测试入口以`../../C5/00_C5导航.md`为准。

## 状态

V5 是在 V4 物理模块和冻结接口基础上形成的**自包含联合模型内核**，当前状态为
`V5_JOINT_MODEL_CORE`。V5 已复制 V4 的 4.1—4.8 边界、接口、物理模型、
高保真子模型、独立评价和证据文件。4.3在 `foundation/modules/4.3_source` 中只保留一份，
由 `source/v5_source_adapter.m` 转换为V5输入。运行、校验和公式追溯均不依赖
`90_历史工作归档/2026-07-31_V4与external归档/`中的V4末版。

V5 核心模型与场景验证层保持分离；`foundation/` 中保留的 V4 测试、文档和图像只作为
来源快照，不是 V5 新结果。C4/V5不再保存活跃场景、联合验证运行器、22方案或验证输出。
这些材料已重构到兄弟项目`../../C5/`，其中5.1和5.2—5.7是活跃入口。
C5不保存V5副本，仅通过相对路径调用本目录。
...
| 小时层容易被误解为构网认证 | 只保留向上注入、向下吸收的对称功率/能量裕度；频率、电压、限流、黑启动另设动态验收门 |
...
第5章48 h场景、22个比例方案、纯电/纯氢/纯算基准、自由分配最优策略以及年度滚动验证
均已迁至`../../C5/`。入口、复现关系和结果口径见`../../C5/00_C5导航.md`；V5只通过
`run_v5_model.m`向C5提供唯一优化内核，不在本目录维护第二套场景或结果。
...
somedata/                         4.3公开资源代理与模型证据；不含C5年度测试数据
manifests/v5_foundation_manifest.csv 4.1—4.8 复制文件哈希清单
tools/sync_v4_foundation.ps1      仅在明确授权从历史V4重建时使用
tools/verify_v5_foundation.ps1    V5 自包含基线普通校验
```

审查结论、参数证据、公式符号和供能快照说明分别见本目录`01`～`03`及`05`文档；
48 h测试接口与判据已迁至`../../C5/5.1场景设计`。
