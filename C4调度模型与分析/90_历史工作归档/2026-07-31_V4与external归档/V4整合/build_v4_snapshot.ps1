param(
    [switch]$VerifyOnly,
    [switch]$RefreshPackageManifestOnly,
    [string]$SourceRoot=''
)
...
function Write-PortablePackageManifest {
    ...
    $package=@(Get-ChildItem -LiteralPath $v4Root -Recurse -File|Where-Object{
        $_.FullName -notmatch $packageExclude
    }|ForEach-Object{
        [pscustomobject]@{
    ...
    Write-Host "V4 PACKAGE MANIFEST REFRESHED: $($package.Count) files"
}
