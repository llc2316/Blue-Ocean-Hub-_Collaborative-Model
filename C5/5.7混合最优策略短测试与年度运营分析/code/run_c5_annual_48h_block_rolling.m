function [summary,windowSummary,results] = ...
    run_c5_annual_48h_block_rolling(outputDir,horizonH)
...
% Compatibility entry point: the former non-overlapping perfect-foresight
% block implementation is retired and all calls route to causal hourly V2.
if nargin<1, outputDir=""; end
if nargin<2, horizonH=[]; end
[summary,windowSummary,hourlyLedger]= ...
    run_c5_annual_causal_hourly(outputDir,horizonH);
results={hourlyLedger};
return
...
if nargin<1 || strlength(string(outputDir))==0
    outputDir=fullfile(c5Root, ...
        '5.7混合最优策略短测试与年度运营分析','results','recomputed');
end
if nargin<2 || isempty(horizonH), horizonH=8760; end
assert(isscalar(horizonH) && isfinite(horizonH) && ...
    horizonH>=24 && horizonH<=8760 && horizonH==round(horizonH), ...
    'horizonH must be an integer in [24,8760].');
...
totalH=numel(base.timeH);
base.pComputeBaseDemandMW=zeros(totalH,1);
base.pComputeFlexibleMaxMW=120*ones(totalH,1);
...
if horizonH<totalH
    base=slice_input(base,1:horizonH,base.initial);
end

starts=(1:48:horizonH)';
n=numel(starts);
results=cell(n,1);
rows=cell(n,1);
state=base.initial;
for w=1:n
    idx=starts(w):min(starts(w)+47,horizonH);
...
den=eE+eH+eC;
summary=table("OPT_FREE_ECON_48H_BLOCK_ROLLING",horizonH,n, ...
