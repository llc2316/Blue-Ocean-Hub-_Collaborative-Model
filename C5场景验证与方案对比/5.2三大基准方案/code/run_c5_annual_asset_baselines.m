function [summary,monthlySummary,results] = ...
    run_v5_product_mix_annual_2025_rolling( ...
    caseIds,outputDir,policy)
%RUN_V5_PRODUCT_MIX_ANNUAL_2025_ROLLING Monthly rolling 8760 h comparison.
%
...
if nargin<1 || isempty(caseIds)
    caseIds=["baseline_E_asset_only","baseline_H_asset_only", ...
        "baseline_C_asset_only","compare_EHC_mixed_optimized"];
end
if nargin<2, outputDir=""; end
if nargin<3 || isempty(policy), policy=struct; end
policy=normalize_policy(policy);

designTable=select_cases(v5_product_mix_specs(),caseIds);
