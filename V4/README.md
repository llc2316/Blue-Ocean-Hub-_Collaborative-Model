...
- 4.6负责PUE、任务和算力服务生产，4.7只负责光缆交付；4.7原生工程中的重复算力、
  制氢、总账和目标函数不进入V4联合入口。
- 默认24小时`interface_smoke`从0 kg储氢开始，当前产氢570.221 kg、交付564.519 kg，电池期末能量严格回到20 MWh、储氢期末为0 kg。电池和储氢循环/最低期末规则只在末时刻施加，逐时路径仍可低于期末目标但不得越过物理边界；长期经济比较仍需统一采用相同初末规则、真实8760 h资源和完整CAPEX/O&M。
