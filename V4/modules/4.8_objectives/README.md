...
- `model/v4_lifecycle_economics_4_8.m`：核算CAPEX、建设期融资、固定运维、折旧、替换和25年NPV。
...
- `docs/4.8全生命周期经济性参数与重算结果.md`：记录公开参数来源、假设边界和24 h/48 h重算结果。

联合入口同时导出最终KPI长表、成本/收入分项和排放分项。最终指标图位于
`outputs/<场景>/figures/final_kpi/`，各指标在完整仿真周期内的变化图位于
`outputs/<场景>/figures/timeseries/`。4.8只提供评价量和分项，不复制4.4总账、
4.5状态递推、4.6任务队列或4.7产品交付逻辑。

当前NPV只用于接口情景与候选方案筛选，不能作为可融资NPV、税后IRR、LCOE、LCOH或产品碳足迹结论。
...
4.8现额外核算期初电池/储氢库存的净消耗机会成本，以及管输/船运实际提取量的分渠道
运输成本，避免“高期初库存、当期零生产、收入却无成本”的账面偏差。当前单价仍为
**[假设值，待企业调研校准]**；若期末库存不低于期初，则不计净库存消耗成本。
