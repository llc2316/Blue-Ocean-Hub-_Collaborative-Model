...
tests={@test_snapshot_hashes,@test_frozen_schema,@test_4_3_existing, ...
    ...
    @test_4_8_evaluation,@test_no_duplicate_ledger,@test_all_channel_48h};
names=["V4包内相对哈希","4.1-4.2冻结Schema","4.3既有模型", ...
    "4.5三时标独立模型","4.3-4.9联合链路","4.9调度约束","缺供与海洋刚性负荷", ...
    "4.8三目标评价","损耗与总账去重","48h全通道行为"];
names=[names(1:6),"4.9 TERMINAL-ONLY PATH RULES", ...
    "4.6 RELEASE TO H2 REDISPATCH",names(7:end)];
...
for name={'v4_hourly_detail.csv','v4_kpi_summary.csv', ...
        ...
        'v4_lifecycle_summary.csv','v4_environment_breakdown.csv'}
...
assert(height(p.service.lifecycleAssetTable)==9);
