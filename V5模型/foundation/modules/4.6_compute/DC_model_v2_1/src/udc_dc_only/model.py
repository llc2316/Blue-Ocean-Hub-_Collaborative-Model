...
def solve_dc_only(bundle: InputBundle, config: dict[str, Any]) -> ModelResult:
    ...
    DC在线时必须满足零任务状态下的基础设施功率；外部请求为0或低于该
    条件底座时，设施停机，刚性任务进入未服务量、柔性任务进入队列，
    可选任务直接丢弃。
    """
    ...
    for t in range(T):
        ...
        bub[r] = cap[t] - base_pdc[t] * float(online[t])
        r += 1
    ...
    p_it[~online] = 0.0
    p_cool[~online] = 0.0
    p_dc_exact[~online] = 0.0
    pue_exact[~online] = np.nan
    p_dc_pwl = base_pdc * online.astype(float) + np.sum(slopes * y, axis=1)
    ...
    hourly = pd.DataFrame(
        {
            ...
            "dc_operational": online.astype(int),
    ...
    audit = {
        ...
        "max_base_power_shortfall_mw": float(
            np.max(np.maximum(base_pdc * online.astype(float) - cap, 0.0))
        ),
    ...
    summary = {
        ...
        "base_power_is_hard_constraint": "CONDITIONAL_WHEN_ONLINE",
        "shutdown_allowed_below_online_base": True,
