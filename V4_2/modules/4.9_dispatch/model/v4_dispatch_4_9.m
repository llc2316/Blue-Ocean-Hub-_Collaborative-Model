...
computeDefault=capacity_request_default(cfg,'computeMW',10);
computeDesired=series_option(options,'computeRequestMW',computeDefault,N); % [假设值，待企业调研校准]
computeEnabled=asset_enabled(cfg,'subseaCompute',true);
if ~computeEnabled, computeDesired(:)=0; end
...
computeDesired(positiveCompute)=max(computeDesired(positiveCompute), ...
    computeOnlineMin);
computeFirmDefault=capacity_request_default(cfg,'computeFirmMW',0);
computeFirm=series_option(options,'computeFirmRequestMW',computeFirmDefault,N);
if ~computeEnabled, computeFirm(:)=0; end
...
h2Default=capacity_request_default(cfg,'hydrogenDeliveryKgPerH', ...
    cfg.hydrogen.deliveryDemandKgPerH);
h2Rate=series_option(options,'h2DeliveryCapKgPerH',h2Default,N);
hydrogenEnabled=asset_enabled(cfg,'PEMElectrolyzer',true);
if ~hydrogenEnabled, h2Rate(:)=0; end
...
if ~asset_enabled(cfg,'powerExportCable',true), activeExportCapMW=0; end
activeElectrolyzerRatedMW=cfg.hydrogen.electrolyzerRatedMW;
if ~hydrogenEnabled, activeElectrolyzerRatedMW=0; end
...
for t=1:N
    ...
    if makeUpMW>0 && remaining>=electrolyzerMinimumMW
        ...
        elyReq(t)=min([requiredMW,activeElectrolyzerRatedMW, ...
    ...
    if hydrogenPriority(t)
        [elyReq(t),remaining]=add_electrolyzer_load( ...
            ...
            activeElectrolyzerRatedMW,electrolyzerMinimumMW);
...
function value=capacity_request_default(cfg,name,defaultValue)
value=defaultValue;
if isfield(cfg,'capacity') && isfield(cfg.capacity,'dispatchRequest') && ...
        isfield(cfg.capacity.dispatchRequest,name)
    value=cfg.capacity.dispatchRequest.(name);
end
end
