...
def main() -> None:
    ...
    # Frozen interface normalization. GPU hours are derived from served MWh-IT
    # with the same public power proxy, so this conversion is an identity at
    # the 4.2 boundary. The business definition of MWh-CS still needs project
    # calibration.
    ...
    h['compute_gross_revenue_yuan']=(
        h['rigid_revenue_yuan']+h['flex_revenue_yuan']+h['spot_revenue_yuan'])
    h['compute_sla_penalty_yuan']=h['total_sla_penalty_yuan']
    h['compute_variable_om_yuan']=h['variable_om_yuan']
    # Internal transfer/opportunity-cost reference only. Source-side project
    # cost is already evaluated in 4.8 and must not be counted twice.
    h['compute_electricity_transfer_yuan']=h['electricity_cost_yuan']
