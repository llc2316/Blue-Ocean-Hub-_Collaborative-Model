...
本包不改动原模块，对必要代码快照，对接V4适配器，以4.4小节为总账，4.8只做评价，4.9做为调度模块，整合4.1—4.9。公共接口固定使用`BLUE_HUB_CH4_SCHEMA_V2 / 2.0.0 / common_case_v2`。联合入口只读取本目录内的`library/`和`modules/`，不依赖项目根目录中的原模块代码。
