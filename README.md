...
| 4.6  | Python桥接直接调用`DC_model_v2_1`的`load_inputs/solve_dc_only`，只做字段与MWh-CS接口映射 | 否             |
| 4.7  | Python桥接直接调用既有`evaluate_power_export`和`evaluate_marine_load`；制氢、PUE、总平衡旧函数不调用 | 否             |
...
当前只读源快照包含4.1、4.2、4.4—4.7以及4.8—4.9目标架构原文，均执行源文件—V4副本SHA-256一致性检查。4.3属于择优合并后的V4派生模块，以来源说明、原生验收和联合验收保证可追溯性与正确性。详细统计见`V4模块纳入与匹配检查报告.md`。
