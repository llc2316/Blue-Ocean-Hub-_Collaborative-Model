...
- `chapter5_preparation/`：第5章22个累计投入电能分配策略、48 h运行器、ENS—消纳率网格、
  阶段汇总和动态构网验收工作点。

V5 活跃测试只保留两类：48 h 短跨度机制测试，以及后续单独建设的 8,760 h 年度测试。
已删除24 h正式测试、旧420 MW容量切分结果和重复说明文件；`foundation/` 中的历史24 h样例
仅是冻结来源证据，不属于V5活跃验证。
...
22个方案共用200 MW海缆、100 MW电解槽和120 MW算力设施，均为
**[假设值，待企业调研校准]**。E/H/C不是装机比例，而是全周期可选投入电能比例：

- E：海缆送端电量；
- H：电解槽输入电量；
- C：柔性算力设施输入电量；基础算力是共同刚性负荷，不进入比例分母。

唯一说明见 `chapter5_preparation/product_mix_design_report.md`。正式结果位于
`outputs/chapter5_product_mix_energy_share_48h/`；48 h全出口机制结果位于
`outputs/v5_all_channel_weather_task_48h/`。
...
chapter5_preparation/              第5章48 h累计能量配比、策略网格与阶段汇总
