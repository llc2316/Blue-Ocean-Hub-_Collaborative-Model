...
% 4) 所有结果与6幅图片写入../docs/assets，供Markdown稳定引用。
...
rootDir = fullfile(modelDir, '..', 'docs', 'assets');
if ~isfolder(rootDir), mkdir(rootDir); end
inputFile = fullfile(modelDir, 'blue_hub_timeseries_input.csv');
...
p.h2Initial = 24000;            % kg，方法情景安全库存
p.h2Price = 30;                 % CNY/kg，经济情景
p.h2VariableCost = 3.5;         % CNY/kg，方法情景打包变动成本
p.bessEnergyCapacity = 40;      % MWh
p.bessInitial = 20;             % MWh
p.bessChargeMax = 20;           % MW
p.bessDischargeMax = 20;        % MW
p.etaCharge = 0.95;
p.etaDischarge = 0.95;
p.curtailPenalty = 30;          % CNY/MWh
p.shortPenalty = 120;           % CNY/kg
p.startCost = 20000;            % CNY/start，方法情景
p.uInitial = 0;
end
...
exportgraphics(f, fullfile(rootDir,'fig01_system_coupling.png'),'Resolution',300,'BackgroundColor','white');
close(f);
...
grid on; box on;
prepare_figure_for_export(f);
exportgraphics(f, fullfile(rootDir,'fig02_electrolyzer_envelope.png'),'Resolution',300,'BackgroundColor','white');
close(f);
...
prepare_figure_for_export(f);
exportgraphics(f, fullfile(rootDir,'fig03_24h_power_dispatch.png'),'Resolution',300,'BackgroundColor','white');
close(f);
...
xlim([1 24]); xticks(1:2:24); grid on; box on;
prepare_figure_for_export(f);
exportgraphics(f, fullfile(rootDir,'fig04_hydrogen_inventory.png'),'Resolution',300,'BackgroundColor','white');
close(f);
...
prepare_figure_for_export(f);
exportgraphics(f, fullfile(rootDir,'fig05_scenario_comparison.png'),'Resolution',300,'BackgroundColor','white');
close(f);
...
prepare_figure_for_export(f);
exportgraphics(f, fullfile(rootDir,'fig06_economic_heatmap.png'),'Resolution',300,'BackgroundColor','white');
close(f);
end


function prepare_figure_for_export(f)
% Make exports independent of the interactive MATLAB light/dark theme.
set(f,'Color','w','InvertHardcopy','off');
ax=findall(f,'Type','axes');
for k=1:numel(ax)
    set(ax(k),'Color','w','XColor','k','YColor','k', ...
        'GridColor',[0.80 0.80 0.80],'MinorGridColor',[0.90 0.90 0.90]);
...
for k=1:numel(lg), set(lg(k),'Color','w','TextColor','k','EdgeColor',[0.3 0.3 0.3]); end
cb=findall(f,'Type','colorbar');
for k=1:numel(cb), set(cb(k),'Color','k'); end
drawnow;
end
