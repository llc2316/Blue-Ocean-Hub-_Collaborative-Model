...
\subsection{在线E/H/C计划份额约束}

在线计划给出的E/H/C份额只作用于每个实际发生可选产品分配的小时，不使用提前已知的全期
总供电量。定义
\begin{equation}
...
\end{equation}
模型保留基础算力变量以支持不可中断SLA敏感性，但第5章主比较将算力全部设为可中断
柔性负荷，基础算力为零。给定因果计划份额
$q_{E,t}+q_{H,t}+q_{C,t}=1$和允许偏差$\delta$，对$i\in\{E,H,C\}$逐时施加
\begin{equation}
...
\label{eq:hourly-mix}
\end{equation}
若$q_{i,t}=0$，则相应产品路径功率为零。$p_{\Sigma,t}=0$时比例约束自然退化为零分配，不
...
内部关键负荷、刚性海洋用能以及启用不可中断SLA场景时的刚性算力分别设置未供变量，并通过目标函数中的高惩罚或
...
固定或分段线性PUE/损耗代理与线性产品比例使运行问题可写成MILP。当前C5采用聚合PUE
输入和固定海缆损耗率；若后续切换到本章高保真分段代理，须在求解后用原物理函数复算，
若复算越界则收紧分段或可行域后重求。
